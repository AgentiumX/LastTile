...
int? solveAStar(Level level) {
  final startTile = level.specialTiles.firstWhere((t) => t.type == TileType.start);
  final endTile = level.specialTiles.firstWhere((t) => t.type == TileType.end);

  final teleportMap = <String, List<(int, int)>>{};
  final keyCount = level.specialTiles.where((t) => t.type == TileType.key).length;
